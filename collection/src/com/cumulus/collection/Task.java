...
public class Task implements Callable<Object> {
    ...
    public Task(User user, List<User> queue, Map<String, User> users, WebClient client, Object lock) {
        this.task = new TaskThread(user, queue, users, client, lock);
    }

    private class TaskThread implements Runnable {

        private User user;
        private List<User> insertQueue;
        private Map<String, User> users;
        private WebClient client;   // WebClient is not thread-safe
        private Object lock;

        public TaskThread(User user, List<User> queue, Map<String, User> users, WebClient client, Object lock) {
        ...
        public void run() {
            try {
                ...
                if (elemsCurCity.size()>0) user.addInfo("current_city", elemsCurCity.get(0).select("div.fsl.fwb.fcb").get(0).getElementsByTag("a").get(0).text());
                if (elemsHometown.size()>0) user.addInfo("hometown", elemsHometown.get(0).select("div.fsl.fwb.fcb").get(0).getElementsByTag("a").get(0).text());
                // fetch basic info
...
//                    System.out.println(myFriend.getHomepage());

                    synchronized (lock) {
                        if (!users.containsKey(myFriend)) {
